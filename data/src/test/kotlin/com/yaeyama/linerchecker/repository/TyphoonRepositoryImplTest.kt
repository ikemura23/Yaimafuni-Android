...
class TyphoonRepositoryImplTest {

    @Test
    fun `fetchTyphoonList propagates the failure instead of swallowing it into an empty list`() = runTest {
        ...
        every { database.getReference("typhoon/tenkijp") } returns reference
        every { reference.addValueEventListener(capture(listenerSlot)) } answers { listenerSlot.captured }

        val repository = TyphoonRepositoryImpl(database)

        repository.fetchTyphoonList().test {
