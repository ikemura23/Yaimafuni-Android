...
- コメントは **日本語** で書く
  - 開発者・利用者ともに日本語話者で、ドメイン用語（航路名・会社名・運航状況）も日本語のため、
    英訳すると意味が伝わりにくくなる。英語化はしない方針とする（#223）
  - 識別子（クラス名・関数名・変数名）は英語。ログ・例外メッセージも英語（Crashlytics で検索しやすくするため）
- 公開 API には KDoc（`/** */`）を書く
  - 対象: domain のモデル・Repository インターフェース・UseCase、data の拡張関数、ViewModel の公開プロパティ・関数、共通コンポーネント
  - プロパティの説明は `//` ではなく `/** */` で書く（IDE のホバーに表示されるため）
  - Flow を返す関数は「いつ値を流すか」「失敗時にどうなるか」を書く
- 実装の説明ではなく「なぜそうしているか」を書く（Firebase のキーに合わせている、表記ゆれを吸収している、など）
- コメントアウトしたコードは残さない（必要なら Git の履歴から戻す）
...
- [ ] 公開 API を追加・変更したら KDoc を書いた／更新したか
