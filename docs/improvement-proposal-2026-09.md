...
`androidApp` / `data` / `domain` / ビルド構成 / CI を横断調査し、改善点を優先度と理由付きで整理したものです。実装の判断はこの提案書のレビュー後に行う前提で、ここでは「何が」「なぜ問題で」「どう直すか」までを記述します。
...
| 優先度 | 件数 | 主な内容 |
|---|---|---|
| P0 | 5 | targetSdk 36 未達、debug 鍵でのリリース署名、R8 完全無効、SDK バージョンの二重管理 |
...
## P0-1. targetSdk 35 が Google Play の更新要件を満たさない
...
**推奨対応**
`compileSdk` / `targetSdk` を 36 へ。ただし P0-2 と同一の作業単位として扱うこと。

**影響範囲**
`build-logic/convention/src/main/kotlin/yaeyama.android.application.gradle.kts`、`gradle/libs.versions.toml`（P0-5 参照）

---

## P0-2. edge-to-edge 未対応のまま targetSdk を上げるとレイアウトが崩れる
...
## P0-3. リリースビルドが debug キーストアで署名されている
...
## P0-5. SDK バージョンが二重管理され、片方が死んでいる
...
**なぜ問題か**
「バージョンカタログを直したのに反映されない」という罠です。P0-1 の作業で最初に踏み抜く可能性が高い。
...
**なぜ問題か**
ビルド時間と APK サイズの無駄です。P0-4（R8 無効）と相まって、使われないコードがそのまま同梱されています。
...
**なぜ問題か**
P0-4 で `shrinkResources` が無効なため、すべて APK に同梱されます。`typhoon_list_empty` のケースは、リソースが存在するのに使われずハードコードが増えるという悪循環の実例です。

**推奨対応**
削除する。P0-4 で `shrinkResources` を有効化すれば今後の検出も容易になる。
...
**推奨対応**
死んだ intent-filter を削除。`ACCESS_NETWORK_STATE` の要否を確認。Predictive Back 対応は P0-1（targetSdk 36）と同じ作業単位で検討する。
...
```
P0-5 (SDK の二重管理を解消)
  └─> P0-1 (targetSdk 36) ─┬─> P0-2 (edge-to-edge)   ← 同一の作業単位
                            └─> P3-15 (Predictive Back)

P3-12 (DTO 層) ──> P0-4 (R8 有効化)   ← Firebase のリフレクションに keep ルールが必要
                    ↑
...
| 段階 | 内容 | 理由 |
|---|---|---|
| 1 | **P2-1 テスト基盤** + **P1-1〜P1-3** | 以降の修正すべてに回帰テストが効くようになる。P1-1〜P1-3 は修正が小さく効果が大きい |
| 2 | **P1-4〜P1-11** | ユーザーに見える不具合の解消。テストを書きながら進める |
| 3 | **P0-5 → P0-1 + P0-2** | ストア更新の再開。edge-to-edge と同時に |
| 4 | **P0-3 署名** + **P2-4〜P2-6 CI** | リリース経路の整備。CI を直してから R8 に触ると検証が楽 |
| 5 | **P3-4, P3-5, P3-3, P3-6 の削除系** | 低リスク。R8 有効化前に消しておくとビルド確認が単純になる |
| 6 | **P0-4 R8** + **P3-12 DTO 層** | keep ルールの検討が必要なため後段 |
| 7 | **P3-2 台風一覧の復活** + **P3-1 M3 移行** + **P3-20 色** | UI 系のまとまった改修 |
| 8 | **P2-7 Dependabot** → **P3-14 バージョン更新** | 継続的な追随体制へ |

P3-9（命名）、P3-17（ナビゲーション）、P3-22（オフライン対応）は方針決定を伴うため、別途検討とします。
## 参考: 調査で確認した「問題ない」点
